...
namespace CommonUtils.ImageHash
{
	...
	public static class ImageAverageHash
	{
		...
		public static ulong AverageHash(Image image)
		{
			...
			Bitmap squeezedImage = ImageUtils.Resize(image, smallerSize, smallerSize);
			...
			uint averageValue = 0;
			byte[] grayscaleByteArray = ImageUtils.ImageToByteArray8BitGrayscale(squeezedImage, out averageValue);

			#if DEBUG
			Image fromBinary = ImageUtils.ByteArray8BitGrayscaleToImage(grayscaleByteArray, smallerSize, smallerSize);
			fromBinary.Save(fileSavePrefix + "3-grayFromArray.png");

			Bitmap grayscaleImage = ImageUtils.MakeGrayscaleFastest(squeezedImage);
